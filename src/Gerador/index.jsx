  ...
  render() {
    return (
      <div style={{ display: "flex", flexDirection: "column" }}>
        ...
        <input
          id="inputNome"
          onChange={(e) => this.handleComponentState(e, "name")}
    ...
    <label htmlFor="inputValue">Valor do Produto do produto</label>
        ...
        <label>Simulação {`${this.state.name}`}</label>
        ...
        <div>
          <span>No toal de: R$ {(this.getValueInstallment() * this.state.installments).toFixed(2)}</span>
        </div>
      </div>
    );
  }
